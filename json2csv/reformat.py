...
#### CUSTOM ROUTINE #####
pathdir = pathlib.Path(r"Obsea_downloadsIFCS\2025_02\19") # be carefull if you have raw .cyz files in your directory you need to skip these!
plist = os.listdir(pathdir)
# plist.remove('raw')

for basename in plist:
    if basename.endswith('.cyz') or basename.endswith('.cyz_images') or basename.endswith('.json_images') or basename.endswith('.csv'):
        continue
    ...
    fcout.extract_parameters_json()
    fcout.write_metadata_csv()
